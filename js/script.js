...
function getSelectedElbow()
{
    const selectElbow = document.getElementById("elbow90").value;

    localStorage.setItem('elbow90numbers',selectElbow);
}



    let haha = localStorage.getItem('diameter')*localStorage.getItem('elbow90numbers');
    console.log(haha);

    //Inicio función Boton calculador

    const pushButton = document.getElementById('calculateButton')

    function resultTotalAreaOut(){
    const outputResult = document.getElementById('supResult');
    const agregar = document.createElement('div');
    agregar.textContent = haha;

    outputResult.appendChild(agregar);
    }    
    
    pushButton.addEventListener('click',resultTotalAreaOut.value);
console()
    //Fin funcion boton calculador
